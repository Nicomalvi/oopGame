...
### `Hitbox`
Por ahora basicamente un rectángulo con un offset por eje.
...
Conoce el MapGrid y las PhysicalEntity, aplica gravedad, mueve las entidades con velocidad != 0 resolviendo colisiones y actualiza la estructura espacial del mapa.
...
### `Actor`
Actúa como una interfaz entre su Behavior y sus Ability: la behavior le dice qué quiere hacer, y el actor traduce esa intención en la ability correspondiente sin que la behavior necesite saber si esa acción existe o puede ejecutarse.

Tiene una PhysicalEntity asociada, un Behavior, y una lista de Ability (movimiento, salto, ataque, etc.). Cada tick, primero les da lugar a sus abilities para que se actualicen solas (por ejemplo, para cortar un salto o terminar un ataque), y recién después ejecuta la behavior.
